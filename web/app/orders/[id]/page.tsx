...
// Horizontal stepper shown in the bottom sheet
          ...
          <div key={step.id} className="flex items-center flex-1 last:flex-none">
            <div className="flex flex-col items-center gap-1">
              ...
              <span className={`text-[9px] font-bold text-center leading-tight whitespace-nowrap ${
                pending ? "text-zinc-300" : active ? "text-zinc-900" : "text-brand-primary"
              }`}>
                {step.label}
              </span>
            </div>
...
export default function OrderTrackingPage() {
  ...
  const etaLabel = isLive ? (eta ?? "On the way") : (hero?.heading ?? order.status);
  const etaSub   = isLive ? "Estimated delivery time" : (hero?.sub ?? "");

  return (
    <div className="min-h-screen bg-white">

      {/* ── Hero (map or status illustration) ── */}
      <div className="relative w-full" style={{ height: "52vh", minHeight: 260 }}>
        ...
        <div className="absolute top-0 inset-x-0 z-50 px-4 pt-safe-top pt-4 flex items-center justify-between">
          ...
          <div className="bg-white/90 backdrop-blur-sm rounded-full px-4 py-1.5 shadow-sm">
      ...
      {/* ── Bottom sheet ── */}
      <div className="bg-white rounded-t-3xl -mt-6 relative z-10 shadow-[0_-6px_24px_rgba(0,0,0,0.07)]">
        ...
        <div className="px-5 pt-3 pb-4 flex items-start justify-between">
          <div>
            <h1 className="text-2xl font-black text-zinc-900 leading-tight">{etaLabel}</h1>
            <p className="text-sm text-zinc-400 mt-0.5">{etaSub}</p>
          </div>
          {isLive && (
            <div className="flex items-center gap-1.5 bg-brand-primary/10 text-brand-primary px-3 py-1.5 rounded-full mt-1">
          ...
          <div className="mx-5 mt-3">
            {order.delivery_otp ? (
              ...
              <div className="bg-zinc-100 rounded-2xl px-5 py-4 flex items-center gap-3">
                ...
                <p className="text-xs font-medium text-zinc-500">OTP will appear here when rider arrives</p>
              </div>
            )}
          </div>
        )}
