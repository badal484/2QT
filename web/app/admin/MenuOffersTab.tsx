...
function TargetSearch({
  targetType, selected, onChange,
}: {
  targetType: string;
  selected: TargetResult[];
  onChange: (items: TargetResult[]) => void;
}) {
  ...
  const multi = targetType === "item"; // only items support multi-select
  ...
  const search = useCallback(async (val: string) => {
    setLoading(true);
    try {
      const data = await api.get(`/admin/offer-targets?type=${targetType}&q=${encodeURIComponent(val)}`);
      setResults((data.results ?? []).filter((r: TargetResult) => !selected.find(s => s.id === r.id)));
      setOpen(true);
    } catch { setResults([]); }
    setLoading(false);
  }, [targetType, selected]);
  ...
  const label: Record<string, string> = { item: "item", kitchen: "kitchen", category: "category" };

  return (
    <div ref={ref} className="relative">
      <div className="text-[9px] font-black uppercase tracking-widest text-zinc-400 mb-1">
        Target {label[targetType]}{multi ? "s (multi-select)" : ""}
      </div>
      ...
      {selected.length > 0 && (
        <div className="flex flex-wrap gap-1.5 mb-2">
          {selected.map(s => (
            <div key={s.id} className="flex items-center gap-1 px-2 py-1 bg-brand-primary/10 border border-brand-primary/20 rounded-lg">
              <div className="w-1.5 h-1.5 rounded-full bg-brand-primary shrink-0" />
              <span className="text-[10px] font-black text-brand-primary max-w-[140px] truncate">{s.name}</span>
              <button onClick={() => remove(s.id)} className="text-brand-primary/60 hover:text-brand-primary ml-0.5">
      ...
      {/* Search input — hide for non-multi once selected */}
      {(multi || selected.length === 0) && (
        <div className="relative">
          ...
          <input
            ...
            onFocus={() => { if (!results.length) search(q); }}
            placeholder={`Search ${label[targetType] ?? "target"} by name…`}
            className="w-full bg-white/[0.05] border border-white/10 rounded-xl pl-8 pr-3 py-2 text-sm font-bold text-white outline-none focus:border-brand-primary/50 placeholder:text-zinc-600"
          />
          {loading && <div className="absolute right-3 top-1/2 -translate-y-1/2 w-3 h-3 border-2 border-brand-primary border-t-transparent rounded-full animate-spin" />}
        </div>
      )}

      {/* Dropdown */}
      {open && (
        <div className="absolute z-50 top-full mt-1 w-full bg-zinc-900 border border-white/10 rounded-xl shadow-2xl overflow-hidden max-h-52 overflow-y-auto">
          {results.length === 0 && !loading && q.length > 0
            ? <div className="px-4 py-3 text-sm text-zinc-500">No {label[targetType]}s found for "{q}"</div>
            : results.map(r => (
...
// ── Audience config sub-fields ────────────────────────────────────────────────
function AudienceConfig({ audience, config, onChange }: {
  audience: string;
  config: any;
  ...
  const num = (label: string, key: string, placeholder: string) => (
    <div>
      ...
      <input type="number" placeholder={placeholder} value={config[key] ?? ""}
        onChange={e => onChange({ ...config, [key]: Number(e.target.value) || undefined })}
  ...
  if (audience === "loyal") return num("Min orders to qualify", "min_orders", "e.g. 5");
  if (audience === "at_risk") return num("Inactive for N+ days", "inactive_days", "e.g. 7");
  if (audience === "churned") return num("Inactive for N+ days", "inactive_days", "e.g. 30");
  if (audience === "high_spenders") return (
    <div>
      <div className="text-[9px] font-black uppercase tracking-widest text-zinc-400 mb-1">Min lifetime spend (paise)</div>
      <input type="number" placeholder="e.g. 100000 = ₹1000" value={config.min_spend_paise ?? ""}
        onChange={e => onChange({ ...config, min_spend_paise: Number(e.target.value) || undefined })}
        className="w-full bg-white/[0.05] border border-white/10 rounded-xl px-3 py-2 text-sm font-bold text-white outline-none focus:border-brand-primary/50"
      />
    </div>
  );
...
const EMPTY_FORM = {
  title: "", description: "",
  target_type: "all",
  discount_type: "percent", discount_percent: "", discount_flat_paise: "",
  max_discount_paise: "",
  audience: "all", audience_config: {} as any,
  zone_id: "", start_time: "", end_time: "", is_active: true,
};

const AUDIENCE_OPTIONS = [
  { value: "all",              label: "Everyone",         desc: "All customers" },
  { value: "new_users",        label: "New Users",        desc: "0 completed orders" },
  { value: "loyal",            label: "Loyal Customers",  desc: "Configurable min orders" },
  { value: "at_risk",          label: "At-Risk",          desc: "Going inactive (configurable days)" },
  { value: "churned",          label: "Churned",          desc: "Long-inactive (configurable days)" },
  { value: "high_spenders",    label: "High Spenders",    desc: "Configurable min lifetime spend" },
  { value: "plus_subscribers", label: "Plus Members",     desc: "Active 2QT Plus subscription" },
];

const targetTypeLabel: Record<string, string> = { all: "All Items", kitchen: "Kitchen", category: "Category", item: "Item" };
const audienceLabel: Record<string, string> = Object.fromEntries(AUDIENCE_OPTIONS.map(o => [o.value, o.label]));
...
export function MenuOffersTab() {
  ...
  const [selectedTargets, setSelectedTargets] = useState<TargetResult[]>([]);
  const [saving, setSaving] = useState(false);

  const load = async () => {
    try {
      const [o, z] = await Promise.all([api.get("/menu-offers/admin"), api.get("/admin/zones")]);
      setOffers(o.offers ?? []);
      setZones(z.zones ?? []);
  ...
  const openEdit = (offer: any) => {
    setEditing(offer);
    // Rebuild selected targets from target_ids or target_id
    const targets: TargetResult[] = offer.target_ids?.length
      ? offer.target_ids.map((id: string, i: number) => ({ id, name: offer.target_names?.[i] ?? id }))
      : offer.target_id
        ? [{ id: offer.target_id, name: offer.target_name ?? offer.target_id }]
        : [];
    ...
    setForm({
      ...
      audience_config: offer.audience_config ?? {},
  ...
  const save = async () => {
    ...
    if (form.target_type !== "all" && selectedTargets.length === 0) {
      toast.error("Please select a target"); return;
    }
    setSaving(true);
    try {
      ...
      const payload = {
        ...
        target_ids: targetIds.length > 1 ? targetIds : undefined,
    ...
    <div>
      ...
      {loading ? (
        ...
        <div className="py-24 text-center border border-white/10 border-dashed rounded-2xl">
          ...
          <p className="text-zinc-500 text-sm font-bold">No menu offers yet.</p>
        </div>
      ) : (
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1 flex-wrap">
                  ...
                  <span className="px-2 py-0.5 rounded-md bg-white/[0.05] text-[9px] font-black uppercase tracking-widest text-zinc-400">
                    ...
                    {offer.target_ids?.length > 1 ? ` (${offer.target_ids.length})` : ""}
                  </span>
                  ...
                  {offer.zone_name
                    ? <span className="flex items-center gap-1 px-2 py-0.5 rounded-md bg-blue-500/10 text-[9px] font-black uppercase text-blue-400"><MapPin className="w-2.5 h-2.5" />{offer.zone_name}</span>
                    : <span className="flex items-center gap-1 px-2 py-0.5 rounded-md bg-white/[0.05] text-[9px] font-black uppercase text-zinc-500"><Globe className="w-2.5 h-2.5" />All Zones</span>
                  }
                </div>
                <div className="text-xs text-zinc-400 font-medium">
                  {offer.discount_type === "percent"
                    ? `${offer.discount_percent}% off${offer.max_discount_paise ? ` · max ₹${offer.max_discount_paise / 100}` : ""}`
                    : `₹${(offer.discount_flat_paise / 100).toFixed(0)} flat off`}
                  {offer.end_time && ` · expires ${new Date(offer.end_time).toLocaleDateString()}`}
                </div>
              </div>
              <div className="flex items-center gap-2 shrink-0">
                <button onClick={() => toggle(offer)} className="text-zinc-400 hover:text-white p-1">
                  {offer.is_active ? <ToggleRight className="w-6 h-6 text-emerald-400" /> : <ToggleLeft className="w-6 h-6" />}
                </button>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-start justify-center p-4 overflow-y-auto">
          <div className="bg-zinc-900 border border-white/10 rounded-2xl w-full max-w-2xl shadow-2xl my-8">
            ...
            <div className="p-6 grid grid-cols-2 gap-5">
              ...
              <div>
                ...
                <select value={form.target_type}
                  onChange={e => { setForm({ ...form, target_type: e.target.value }); setSelectedTargets([]); }}
                  className="w-full bg-zinc-900 border border-white/10 rounded-xl px-3 py-2 text-sm font-bold text-white outline-none focus:border-brand-primary/50">
              ...
              {/* ── Audience ── */}
              <div>
                <div className="text-[9px] font-black uppercase tracking-widest text-zinc-400 mb-1">Audience</div>
                <select value={form.audience}
                  onChange={e => setForm({ ...form, audience: e.target.value, audience_config: {} })}
                  className="w-full bg-zinc-900 border border-white/10 rounded-xl px-3 py-2 text-sm font-bold text-white outline-none focus:border-brand-primary/50">
              ...
              {/* Audience config sub-field */}
              <div>
                <AudienceConfig
                  audience={form.audience}
                  config={form.audience_config}
                  onChange={cfg => setForm({ ...form, audience_config: cfg })}
                />
              </div>
            ...
            <div className="flex gap-3 p-6 pt-0">
              <button onClick={() => setShowModal(false)}
                className="flex-1 py-3 rounded-xl bg-white/[0.05] text-sm font-black text-white hover:bg-white/10">
                Cancel
              </button>
              <button onClick={save} disabled={saving}
                className="flex-1 py-3 rounded-xl bg-brand-primary text-white text-sm font-black hover:bg-brand-dark disabled:opacity-60 shadow-lg shadow-brand-primary/30">
