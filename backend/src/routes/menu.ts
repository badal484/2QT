    ...
    try {
        ...
        if (apiKey) {
            try {
                ...
                console.warn('[Geocode] Google Places API error, falling back to Nominatim:', data.status, data.error_message);
            } catch (err) {
                console.warn('[Geocode] Google Places API fetch failed, falling back to Nominatim');
            }
        }
    ...
    const [itemsResult, zoneResult, kitchenResult] = await Promise.all([
        query(
            `SELECT id, name, description, price_paise, photo_url, is_veg, is_egg, available,
                    ...
                    is_bestseller, is_new, tags, customization_groups
