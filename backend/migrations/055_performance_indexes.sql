...
CREATE INDEX IF NOT EXISTS idx_menu_items_zone_active ON menu_items(zone_id, available) WHERE available = true;
...
CREATE INDEX IF NOT EXISTS idx_orders_kitchen_delivered ON orders(kitchen_id, status) WHERE status = 'delivered';
...
CREATE INDEX IF NOT EXISTS idx_addresses_not_deleted ON addresses(customer_id) WHERE deleted_at IS NULL;

-- rider_daily_earnings: payout cron date range
CREATE INDEX IF NOT EXISTS idx_rider_daily_earnings_date ON rider_daily_earnings(rider_id, date DESC);

-- weekly_payouts: pending payout lookup
CREATE INDEX IF NOT EXISTS idx_weekly_payouts_status ON weekly_payouts(status, rider_id);

-- kitchen_payouts: pending payout lookup
CREATE INDEX IF NOT EXISTS idx_kitchen_payouts_status ON kitchen_payouts(status, kitchen_id);
