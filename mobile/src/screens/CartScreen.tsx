...
import { setQuantity, clearCart, setInstructions, setScheduledAt } from '../store/slices/cartSlice';
...
const CartScreen = ({ navigation }: any) => {
  ...
  const p = pricingData?.pricing || { 
    subtotalPaise: items.reduce((acc, item) => acc + item.quantity * item.pricePaise, 0),
    deliveryFeePaise: 0,
    gstPaise: 0,
    totalAmountPaise: 0,
    discountPaise: 0,
      ...
      <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
        <View style={styles.scrollContent}>
          ...
          {items.map((item) => (
            <View key={item.menuItemId} style={styles.itemCard}>
              ...
              <View style={styles.quantityControl}>
                <TouchableOpacity onPress={() => dispatch(setQuantity({ menuItemId: item.menuItemId, quantity: item.quantity - 1 }))}>
          ...
          <View style={styles.scheduleRow}>
            <TouchableOpacity 
              onPress={() => dispatch(setScheduledAt(null))}
            ...
            <TouchableOpacity 
              onPress={() => {
                ...
                dispatch(setScheduledAt(tomorrow.toISOString()));
              }}
              style={[styles.scheduleBtn, scheduledAt ? styles.scheduleBtnActive : styles.scheduleBtnInactive]}
            >
